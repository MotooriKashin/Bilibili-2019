...
/** 底部弹幕 */
@customElement('div')
export class Mode4 extends Mode0 {
     ...
     * @returns 是否允许添加
     */
    private $add(layer: Mode4[], from: number, to: number) {
        ...
        this.style.translate = `0 ${this.$container.$heightFix - to}px`;
    ...
    async execute(delay = 0) {
        if (this.$render) {
            ...
            this.$space();
            this.style.animation = `0ms linear ${delay + this.$duration}ms both dmMode4`;
            this.$wraps > 1 || this.$container.$danmakuNow++;
        }
    }
}