...
/** 逆向弹幕 */
@customElement('div')
export class Mode6 extends Mode0 {
    ...
    private $space() {
        if (this.$wraps < 2) {
            ...
            for (let i = 0; i <= Mode6.space.length; i++) {
                if ($danmakuArea && i) {
                    return this.$cancel();
                }
                if (!Mode6.space[i]) {
                    ...
                    Mode6.space.push(layer);
                    break;
                } else if (this.$layer(Mode6.space[i])) {
                    // 弹幕层可以进入
                    break;
                }
            }
     ...
     * @returns 是否允许添加
     */
    private $add(layer: Mode6[], from: number, to: number) {
        const set = new Set<Mode6>();
    ...
    async execute(delay = 0) {
        if (this.$render) {
            ...
            this.$space();
            this.style.animation = `${this.$duration}ms linear ${delay}ms both dmMode6`;
        }
    }
}